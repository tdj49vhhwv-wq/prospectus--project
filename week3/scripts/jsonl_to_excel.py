...
def jsonl_to_excel(path, out_path):
    ...
    h1 = ["PDF页码", "增资日期", "认购方", "认购数量(万股)", "认购金额(万元)",
          "认购价格(元/股)", "事件类型", "增资后总股本(万股)", "认购占比", "数据来源", "原文证据"]
    _header(ws1, h1)
    for r in sub_flows:
        ws1.append([
            ...
            r.get("data_source", "pdf_disclosed"),
    ...
    sep_cell = ws3.cell(row=row, column=1, value="=== 以下为交叉校验结果 ===")
    sep_cell.font = Font(bold=True, size=12, color="FF0000")
    sep_cell.fill = PatternFill(start_color="FFFFCC", end_color="FFFFCC", fill_type="solid")
    row += 3
    ...
    row += 3
    ws3.merge_cells(f"A{row}:D{row}")
    stat_header = ws3.cell(row=row, column=1, value="=== 汇总统计 ===")
    stat_header.font = Font(bold=True, size=12, color="0000FF")
    stat_header.fill = PatternFill(start_color="CCE5FF", end_color="CCE5FF", fill_type="solid")
    row += 2
