...
def split_names(raw):
    """
    Split a narrative syndicate member list while rejecting
    structural noise inherited from the preceding table/date row.

    Important:
    - generic rules only;
    - no company-specific first-investor hardcoding;
    - declared_count remains the final consistency gate.
    """

    raw = norm_space(raw)

    # Repair MinerU/PDF line wraps inside Chinese organization names.
    raw = re.sub(
        r"(?<=[\u4e00-\u9fff])\s*\n\s*(?=[\u4e00-\u9fff])",
        "",
        raw,
    ...
    # Normalize final conjunction to the ordinary list separator.
    raw = re.sub(
        r"[及和]\s*(?=[^、，,；;]{2,40}$)",
        "、",
        raw,
    ...
    for x in parts:
        ...
        # Date-prefixed structural fragment.
        if re.match(
        ...
        # -----------------------------------------------
        # 2. Reject numeric/table residue.
        #
        # Examples:
        #   000.0000 100.00 7
        #   100.00
        ...
        #   36,000.00 100.00
        #
        # Keep this generic: if removing numeric syntax and
        # whitespace leaves no Chinese/Latin letters, it is
        # not an investor name.
        # -----------------------------------------------
        ...
        # -----------------------------------------------
        # 3. Reject common table headers / structural labels.
        # -----------------------------------------------
        ...
        if compact in structural_labels:
            continue

        # -----------------------------------------------
        # 4. Reject obvious narrative fragments.
        # -----------------------------------------------
        ...
        # -----------------------------------------------
        # 5. Extremely long fragments are probably prose,
        # not organization/person names.
        # -----------------------------------------------
        if len(compact) > 40:
            continue

        # -----------------------------------------------
        # 6. Require at least one Chinese/Latin character.
        # -----------------------------------------------
        if not re.search(
            r"[\u4e00-\u9fffA-Za-z]",
            compact,
