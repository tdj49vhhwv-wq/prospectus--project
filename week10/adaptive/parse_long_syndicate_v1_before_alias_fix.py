...
def parse_alias_definitions(text):
    """
    Capture:
      A、B、C ... 及 Z（以下简称“Pre-IPO轮股东”）共计38家主体
    """

    aliases = []

    # Keep the window bounded so we do not swallow previous tables.
    pat = re.compile(
        r"(?P<members>"
        r"[\u4e00-\u9fffA-Za-z0-9（）()·\-、，,\s]{20,2500}?"
        r")"
        r"[（(]\s*以下简称\s*[“\"]"
        r"(?P<alias>[^”\"]{2,40})"
        r"[”\"]\s*[）)]"
        r"\s*共计\s*(?P<count>\d+)\s*家(?:主体)?",
        re.S,
    )
    ...
    for m in pat.finditer(text):
        ...
        # Trim to the latest sentence/paragraph boundary.
        cuts = [
            raw.rfind("。"),
            raw.rfind("；"),
            raw.rfind("\n\n"),
        ...
        # Remove leading date/introduction.
        raw = re.sub(
            r"^\s*\d{4}\s*年[^，,。；;]{0,30}[，,]\s*",
            "",
            raw,
