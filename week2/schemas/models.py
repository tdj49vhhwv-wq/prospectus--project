"""
Pydantic v2 模型 — Week 3 三类基础事实记录

1. SubscriptionFlow    — 认缴流量: 谁在什么时候认购了多少、多少钱、什么价格
2. ShareTransferFlow   — 股权转让流量: 转让方、受让方、数量、价格/价款
3. EquitySnapshot      — 股权结构存量: 某个时点的股东结构是什么
...
class RecordType(str, Enum):
    ...
    SHARE_TRANSFER_FLOW = "share_transfer_flow"
...
class EventContext(str, Enum):
    ...
    CAPITAL_RESERVE_CONVERSION = "资本公积转增"
    VIE_SETUP = "VIE搭建"
    VIE_REMOVAL = "VIE拆除"
...
class SubscriptionFlow(BaseModel):
    ...
    source_page: str = Field(
        ...,
        min_length=1,
        description="PDF 页码或 MD 文件行号范围（如: 友升股份2.md L800-L830, PDF p43-45）"
    )
    ...
    data_source: DataSource = Field(
        default=DataSource.PDF_DISCLOSED,
        description="数据来源: pdf_disclosed(原文直接披露) / calculated(计算得出) / inferred(推断)"
    )
...
# ============================================================
# 2. 股权转让流量 — ShareTransferFlow
# ============================================================

class TransferType(str, Enum):
    PROXY_RESTORE = "代持还原"
    SAME_CONTROL = "同一控制下转让"
    EXTERNAL = "外部转让"
    ZERO_PRICE = "零对价转让"
    ACQUISITION = "收购"
    OTHER = "其他"
...
class ShareTransferFlow(BaseModel):
    """
    股权转让流量: 回答"谁把多少股权以什么价格转给了谁"

    每行 = 一对转让方→受让方的转让记录。
    """
    record_type: RecordType = Field(
        default=RecordType.SHARE_TRANSFER_FLOW,
        description="记录类型标识"
    )
    company_name: str = Field(
        ...,
    ...
    source_page: str = Field(
        ...,
        min_length=1,
        description="PDF 页码（如: PDF p43-45）"
    )
    ...
    transfer_date: str = Field(
        ...,
        min_length=7,
        description="转让日期（YYYY-MM-DD 或 YYYY-MM）"
    )
    transferor_name: str = Field(
        ...,
        min_length=1,
        description="转让方名称（谁卖出）"
    )
    transferee_name: str = Field(
        ...,
        min_length=1,
        description="受让方名称（谁买入）"
    )

    # ── 数量/价格（可为空，不强行倒推） ──
    shares_transferred: Optional[float] = Field(
        None, ge=0,
        description="转让数量（万股）。PDF 未披露则留空"
    )
    transfer_amount: Optional[float] = Field(
        None, ge=0,
        description="转让价款/总价（万元）。PDF 未披露则留空"
    )
    price_per_share: Optional[float] = Field(
        None, ge=0,
        description="每股转让价格（元/股）。PDF 未披露则留空"
    )
    transfer_ratio: Optional[str] = Field(
        None,
        description="转让股权占比（如 0.80%）"
    )

    transfer_type: Optional[TransferType] = Field(
        None,
        description="转让类型（代持还原/同一控制下/外部转让/零对价/收购/其他）"
    )
    ...
    @model_validator(mode="after")
    def cross_check_price_amount(self) -> "ShareTransferFlow":
        """价格×数量≈金额 一致性检查"""
        if self.price_per_share and self.shares_transferred and self.transfer_amount:
            expected = self.price_per_share * self.shares_transferred
            if self.transfer_amount > 0 and abs(expected - self.transfer_amount) / self.transfer_amount > 0.06:
                pass  # 允许6%误差
        return self


# ============================================================
# 3. 股权结构存量 — EquitySnapshot
# ============================================================
