...
PATTERNS = [
    ...
    # 共同增资/增至
    (r'(\d{4}\s*年\s*\d{1,2}\s*月(?:\s*\d{1,2}\s*日)?)[^。；]{0,80}?(?:共同)?增资\s*([\d,]+\.?\d*)\s*万元.*?'
    ...
    # 增资：第N次增资X万元，注册资本增至Y
    (r'(\d{4}\s*年\s*\d{1,2}\s*月(?:\s*\d{1,2}\s*日)?)[^。；]{0,30}?第[一二三四五六七八九十]+次增资[^。；]{0,80}?([\d,]+\.?\d*)\s*万元[^。；]{0,200}?注册资本[增至为:：]*\s*([\d,]+\.?\d*)\s*万元', '增资'),
    # 增资：增资至X万元
    ...
    # 股权转让：X%股权分别转让予（多受让方名单）
    (r'(\d{4}\s*年\s*\d{1,2}\s*月(?:\s*\d{1,2}\s*日)?)[^。；]{0,150}?将其(?:各自)?持\s*有\s*的.*?(\d+(?:\.\d+)?)%\s*股权.*?分别转\s*让\s*予\s*([\s一-龥、]{2,40}?)', '股权转让'),
