...
ALIAS_GROUPS = [
    ...
    ["深圳市达晨创程私募股权投资基金企业(有限合伙)","达晨创程","深圳达晨创程"],
    ["杭州达晨创程股权投资基金合伙企业(有限合伙)","杭州达晨创程"],
    ["海南三亚达晨财汇私募股权投资基金合伙企业(有限合伙)","达晨财汇"],
    ["深圳赛格高技术投资股份有限公司","赛格高技术"],
    ["SAIC TECHNOLOGIES FUND II,LLC","上汽科技","SAIC"],
]

BAD_AUTO = (
    "议案","审议","认购人","认购数量","认购金额","新增股本","新增股份",
    "注册资本","股本","股东大会","董事会","协议","报告","合计","总计","（待识别）",
    "关于","情况如下","价格为","数量","金额"
)
...
def date_ok(a,b):
    ...
    return bool(a[:7]) and a[:7] == b[:7]

def event_code(v):
    s = str(v or "")
    return EVENT_CODE.get(s, s)
...
def in_master_event(r, master):
    ...
    date = r.get("event_date") or r.get("subscription_date")
    typ = event_code(r.get("event_type") or r.get("event_context"))
    for m in master:
        if str(m.get("stock_code")) != code:
            continue
        if not date_ok(date, m.get("date")):
            continue
        if typ != str(m.get("type","")):
            continue
        return True
    return False
...
def main():
    ...
    for p in sorted(args.auto_dir.glob("*_subscription_flow.jsonl")):
        for line in p.read_text(encoding="utf-8").splitlines():
            if not line.strip():
                continue
            x=json.loads(line)
            name=str(x.get("subscriber_name",""))
            r={
                "stock_code":x.get("stock_code",""),
                "event_date":x.get("subscription_date",""),
                "event_type":event_code(x.get("event_context","")),
            ...
            if not in_master_event(r,master):
                continue
            if any(b in name for b in BAD_AUTO):
                continue
            auto.append(r)
    ...
    for gi,g in enumerate(gold):
        for ai,a in enumerate(auto):
            if ai in used:
                continue
            if g["stock_code"]!=a["stock_code"]:
                continue
            if event_code(g["event_type"])!=event_code(a["event_type"]):
                continue
            if not date_ok(g["event_date"],a["event_date"]):
                continue
    ...
    for gi,g in enumerate(gold):
        hit=mg.get(gi)
        rows.append({
            "role":"gold","status":"TP" if hit else "FN",
            "stock_code":g["stock_code"],"event_date":g["event_date"],
            "event_type":event_code(g["event_type"]),
            "raw_name":g["subscriber_name"],
            "canonical_name":canon(g["subscriber_name"]),
            "matched_name":auto[hit[0]]["subscriber_name"] if hit else "",
            "match_method":hit[1] if hit else "",
        })

    for ai,a in enumerate(auto):
        hit=ma.get(ai)
        rows.append({
            "role":"auto","status":"TP" if hit else "FP",
            "stock_code":a["stock_code"],"event_date":a["event_date"],
            "event_type":event_code(a["event_type"]),
            "raw_name":a["subscriber_name"],
            "canonical_name":canon(a["subscriber_name"]),
            "matched_name":gold[hit[0]]["subscriber_name"] if hit else "",
            "match_method":hit[1] if hit else "",
        })
    ...
    summary={
        "gold":len(gold),"auto":len(auto),"matched":tp,
    ...
    print("========================================")
    print(" Stage 7 PE/VC Evaluation v2 FIXED")
    print("========================================")
